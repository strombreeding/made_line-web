...
import styles from "../../../styles/PnlTable.module.css";
import { useGlobalStore } from "../../../store/globalStore";
import { useEffect, useState } from "react";
import NewAccount from "../../modal/NewAccount";
import { usePnlStore } from "../../../store/pnlStore";
import PnlTableFilter from "./PnlTableFilter";
import { IPnlTableData } from "../../../types/pnl";
export default function PnlTable() {
  const { pnlTableList, setPnlTableList, setPageInfo, pageInfo } = usePnlStore(
    (state) => state
  );
  ...
  const req = async (locationFilter = false) => {
    const res = await fetch("/api/pnl/profit");
    const data = await res.json();

    if (locationFilter) {
      if (selectLocation === "전체지점") {
        setPnlTableList(data);
      } else {
        const newList = data.filter(
          (user: IPnlTableData) => user.location === selectLocation
        );
        setPnlTableList(newList);
      }
    } else {
      setPnlTableList(data);
    }
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      >
        <PnlTableFilter setDashboard={setPnlTableList} setReady={setReady} />
      </div>
      ...
      <div className={styles.tableWrapper}>
        ...
        {/* 데이터 */}
        {pnlTableList.map((item, i) => {
          return (
            <div
              className={styles.tableBodyWrapper}
              key={i}
              style={{
                borderBottomLeftRadius: pnlTableList.length - 1 === i ? 20 : 0,
                borderBottomRightRadius: pnlTableList.length - 1 === i ? 20 : 0,
              }}
            >
